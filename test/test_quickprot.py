...
def test_quickprot_transdecoder_saccharomyces(
    ...
    quickprot_cmd_transdecoder = quickprot_cmd + ("--prefix", temp_dir / "quickprot")
    print(quickprot_cmd_transdecoder)
    run(quickprot_cmd_transdecoder, check=False)
...
def test_quickprot_td2_saccharomyces(
    ...
    quickprot_cmd_td2 = quickprot_cmd + ("--prefix", temp_dir / "quickprot", "-ORFSoftware", "TD2",)
    print(quickprot_cmd_td2)
    run(quickprot_cmd_td2, check=False)
    for suffix in output_file_suffixes[-1:]:
        test_file = f'{temp_dir / "quickprot"}.{suffix}'
        reference_file = f'{output_reference_dir / "quickprot"}.{suffix}'
        print(test_file, reference_file)
        with open(test_file, "rb") as tf, open(reference_file, "rb") as rf:
