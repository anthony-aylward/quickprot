...
def check_dependencies(ORFSoftware, miniprot_PATH=None, TransDecoder_PATH=None):
    ...
    print("")
    return miniprot_PATH, TransDecoder_PATH, ORFSoftware
...
def run_miniprot(miniprot_PATH, query_file, genome_file, output, thread, mask, skip_align, outs, genetic_code, splice_model, debug_info):
    if mask:
...
def transcript_assembly(miniprot_output, identity, cover, prefix, query_file, preserve_the_starting_AA_number=0, align_number=0, in_stop_number=0):
    ...
    if preserve_the_starting_AA_number==0:
        ...
        if query_file.endswith('.gz'):
            for record in SeqIO.parse(gzip.open(query_file, mode='rt'), 'fasta'):
                target_AA_dict.setdefault(record.id, str(record.seq[:preserve_the_starting_AA_number].upper()))
        else:
...
the fused ORF will be split in subsequent analysis.""")
    ...
    miniprot_PATH, TransDecoder_PATH, ORFSoftware = check_dependencies(ORFSoftware, miniprot_PATH, TransDecoder_PATH)
